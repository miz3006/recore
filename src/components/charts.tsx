...
/**
 * Chart primitives (CLAUDE.md §8). One rule inherited from the two-ink system:
 * history is white at low alpha, THE PRESENT IS SIGNAL. No gridlines, no
 * legends, no decoration — a bar, a value, a hairline baseline. Everything is
 * sized in moderateScale so charts stay proportional across devices.
 */
...
/**
 * The 8-week volume chart: current week in signal with its value on top,
 * history in quiet white. Zero weeks stay honestly empty (no 3px fake bars).
 */
            ...
            {data.map((d, i) => {
              ...
              return d.value > 0 ? (
                <Rect
                  ...
                  height={h}
                  rx={BAR_RADIUS}
                  fill={current ? color.signal : alpha(color.accent, ink.rule)}
                />
              ) : null;
            })}
          </Svg>
 ...
 * in low-alpha white, this week in signal.
 */
      ...
      {data.map((v, i) => {
        ...
        return (
          <Rect
            ...
            height={h}
            rx={1.5}
            fill={i === n - 1 && v > 0 ? color.signal : alpha(color.accent, v > 0 ? ink.rule : 0.1)}
          />
        );
      })}
    </Svg>
...
/**
 * A quiet trend line (e1RM over sessions): hairline white path, the latest
 * point marked in signal. No axes — the numbers live next to it.
 */
export function Sparkline({
  ...
  return (
    <Svg width={width} height={height}>
      ...
      <Circle cx={lastX} cy={lastY} r={3} fill={color.signal} />
    </Svg>
  );
}

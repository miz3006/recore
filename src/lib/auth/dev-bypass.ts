...
/**
 * The development bypass for the hard paywall. `__DEV__` only.
 *
 * Lets a dev past the paywall with NO account, so the app can be worked on
 ...
 * where the data lives. Signing in properly later takes over from here (the
 * real user id replaces the dev id and `ensureLocalUser` rescopes the mirror).
 *
 * This skips the SCREEN, never the account or an entitlement — it is not a
 * purchase stub. `__DEV__` compiles the whole path out of release bundles, and
 * the flag is read through it everywhere, so a shipped build can never be in
 * this state.
 */

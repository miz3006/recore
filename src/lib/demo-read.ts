 ...
 *
 *   1. repeated `W×R` pairs      100x8 90x10 80x12 · 140x5, 160x3, 180x1
 *   2. `W×R×S`                   185x5x3 · 100kg x12 x3
 *   3. one `A×B`                 3x8 · 5x5 · 100kgx5 · 15x2
 *   4. a rep list                100kg 5,5,4 · 80kg 8/7/6 · 12,10,8
 *   5. spaced reps               100kg 8 8 8
 *   6. words                     4 serije po 10 · 3 sets of 8 · 5 ponovitev
 *   7. one count                 bench 5 80kg · deadlift 180 x3 · curls 12 15kg
 *   8. distance and time         400m · 2,5km · 4x50m · plank 60s · 30 min
 *
...
const LIFT_ALIASES: Record<string, readonly string[]> = {
  ...
  'Pull-ups': ['pull up', 'pull ups', 'pullup', 'pullups', 'pull-up', 'pull-ups', 'chin up', 'chin ups', 'chinup', 'chinups', 'zgibi'],
  'Barbell row': ['row', 'rows', 'barbell row', 'bb row', 'bent over row', 'pendlay row', 'veslanje z drogom'],
};
...
const KIND_WORDS: { test: RegExp; kind: SetKind }[] = [
  { test: /\b(warm\s?ups?|warmup|ogrevanj\w*)\b/i, kind: 'warmup' },
...
const NAME_NOISE = new Set([
  'bw', 'bodyweight', 'just', 'the', 'bar', 'each', 'na', 'nogo', 'per', 'side',
  'prazna', 'palica', 'sem', 'naredil', 'danes', 'z', 's', 'in', 'and', 'with', 'x',
]);
...
const KIND_IN_NAME = /\b(warm\s?ups?|warmup|ogrevanj\w*|drop\s?sets?|dropset|myo(?:reps)?|amrap|to failure|do odpovedi)\b/gi;
...
function readSegment(segment: string): SegmentRead | null {
  ...
  const unit: 'kg' | 'lb' = /\d\s*(?:lbs?|pounds?)\b/i.test(text) ? 'lb' : 'kg';
  /**
...
function readName(raw: string): { name: string | null; continuation: boolean } {
  ...
  if (!head) {
    ...
    return tail ? { name: tail, continuation: false } : { name: null, continuation: true };
  }

  const words = head.toLowerCase().split(' ');
  if (CONTINUATIONS[words[0]!] !== undefined) return { name: null, continuation: true };

  // Strip the words that are never part of a movement's name, from the END
  // inwards: "dips bw+20" is Dips, "bench prazna palica" is Bench.
  const kept = [...words];
  while (kept.length > 1 && NAME_NOISE.has(kept[kept.length - 1]!)) kept.pop();
  while (kept.length > 1 && NAME_NOISE.has(kept[0]!)) kept.shift();
  const cleaned = kept.join(' ').trim();
  if (cleaned.length < 2) return { name: null, continuation: true };

  return { name: canonicalName(cleaned) ?? titleCase(cleaned), continuation: false };
}
...
function readSets(text: string, unit: 'kg' | 'lb'): ReadSet[] {
  ...
  const pairs = [...text.matchAll(PAIR)].map((m) => ({
    left: toNumber(m[1]!),
    leftUnit: unitOf(m[2]),
    right: Number.parseInt(m[3]!, 10),
  }));
  if (pairs.length >= 2) {
    const sets = fromPairs(pairs, unit, kg, load);
    if (sets.length > 0) return sets;
  }
  ...
  const list = REP_LIST.exec(text);
  if (list && load != null) {
    const reps = repsIn(list[0]);
    if (reps.length > 1) return reps.map((r) => set({ reps: r, weightKg: load }));
  }
  ...
  if (list) {
    const reps = repsIn(list[0]);
    if (reps.length > 0) return reps.map((r) => set({ reps: r, weightKg: load }));
  }
  ...
  if (load != null && bare.length >= 3 && bare.every((n) => n.value >= 1)) {
    return bare.map((n) => set({ reps: n.value, weightKg: load }));
  }
...
const TRIPLE = /(\d+(?:[.,]\d+)?)\s*(kgs?|lbs?|kg|lb)?\s*[x×]\s*(\d+)\s*[x×]\s*(\d+)\b/i;
 ...
 */
function loadOf(text: string, unit: 'kg' | 'lb', used: number[]): number | null {
  const numbers = numbersIn(text);
  ...
  const bare = numbers
    .filter((n) => n.unit == null && !spent.has(n.value) && n.value >= LOAD_FLOOR)
    .map((n) => n.value)
    .filter((v) => inLoadRange(v, unit));
  if (bare.length === 0) return null;
  const top = Math.max(...bare);
...
function normalise(raw: string): string {
  return (
    String(raw ?? '')
      ...
      // "bw+20" is twenty kilograms on a belt, not a rep count.
      .replace(/\bbw\s*\+\s*(\d+(?:[.,]\d+)?)/gi, '$1kg')
...
/** A separator with WORDS after it — a second movement on the line. */
const NEW_MOVEMENT = /\s*(?:;|\+|,|\band\b|\bin\b|\bss\b|\bsuperset\b)\s+(?=\p{L}{2})/giu;

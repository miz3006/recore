 ...
 * actually improving?").
 *
 * The screen shows one card per lift: a step series over the chosen range, the
 ...
 *    the range there is nothing to draw, so the lift gets no card rather than a
 *    flat line pretending to be a plateau.
...
export interface ProgressionView {
   ...
   * the truth, the same rule the Lifts tab follows). */
...
export function buildProgression(
  ...
  for (const sessions of byLift.values()) {
    ...
    if (points.length < MIN_SESSIONS_FOR_CARD || !latestRow) continue;

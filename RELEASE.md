...
| **Health & Fitness** | **No — nothing is collected.** Since 17 Sep 2026 the binary carries the `com.apple.developer.healthkit` entitlement and Recore WRITES finished sessions to Apple Health as workouts (start, end, kind of training). It requests no read permission and holds none, so no health data is read, received, stored or transmitted — writing to a person's own Health store is not collection. `NSHealthUpdateUsageDescription` states the purpose; `src/lib/health/` is the only code that touches HealthKit. |
| Location, Contacts, Photos, Purchases, Search, Browsing, Advertising ID | **No** |
